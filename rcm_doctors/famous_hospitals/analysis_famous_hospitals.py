...
def save_famous_hospitals_from_mysql(out_file):
    ...
    sql = "select hospital_id,norm_name,is_deleted from symptomchecker_goodhospital"
    cursor.execute(sql)
    rows = cursor.fetchall()
    hsp_id_name = dict()
    for row in rows:
        hospital_id = unicode(row[0])
        norm_name = unicode(row[1])
        is_deleted = row[2]
        if is_deleted == 0:
            hsp_id_name[hospital_id] = norm_name
    hsp_list = sort_dict_by_value(hsp_id_name)
    for (hsp_id, hsp_name) in hsp_list:
        # data_line = ','.join([hsp_id, hsp_name])
        write_line(out_file, hsp_id)
...
def load_famous_hospitals_from_mysql(file_name):
    ...
    res_set = set()
    for data_line in data_lines:
        [hsp_id, hsp_name] = data_line.split(',')
        res_set.add(unicode(hsp_id))
    return res_set
