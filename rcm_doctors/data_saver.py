...
def save_train_data(start_date, end_date, log=False):
    ...
    connection = happybase.Connection(host=HBaseConfigs.hbase_host, port=HBaseConfigs.hbase_port)
    rows = connection.table(HBaseConfigs.search_log_table).scan(row_start=start_date, row_stop=end_date)
